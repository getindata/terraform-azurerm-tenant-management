...
| <a name="input_create_default_ad_groups"></a> [create\_default\_ad\_groups](#input\_create\_default\_ad\_groups) | Indicates wherever the default AD groups should be created for the subscription | `bool` | `false` | no |
| <a name="input_default_ad_groups_for_resource_containers"></a> [default\_ad\_groups\_for\_resource\_containers](#input\_default\_ad\_groups\_for\_resource\_containers) | Map of default AD groups that will be created for the subscription if enabled | <pre>map(object({<br>    role_names : list(string)<br>  }))</pre> | `{}` | no |
